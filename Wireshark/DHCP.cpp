...
public:
    // Constructor
    ICMPFilter(std::string n, bool e, std::string sp, std::string dp, std::string sm, std::string dm, int t, int c, int pr, uint16_t ps, bool b) : Filter(n, e, sp, dp, sm, dm), dhcpType(t), dhcpCode(c), packetRate(pr), payloadSize(ps), isBroadcast(b) {}
